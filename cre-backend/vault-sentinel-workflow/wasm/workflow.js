...
var exchangeSchema = exports_external.object({
  exchangeId: exports_external.string(),
  name: exports_external.string(),
  reserveApiUrl: exports_external.string(),
  walletTrackerUrls: exports_external.array(exports_external.string())
});
var evmConfigSchema = exports_external.object({
  chainName: exports_external.string(),
  porFeedAddresses: exports_external.array(exports_external.string())
});
var alertThresholdsSchema = exports_external.object({
  healthyMin: exports_external.number(),
  warningMin: exports_external.number(),
  criticalMin: exports_external.number()
});
var configSchema = exports_external.object({
  schedule: exports_external.string(),
  exchanges: exports_external.array(exchangeSchema),
  evms: exports_external.array(evmConfigSchema),
  alertThresholds: alertThresholdsSchema
});
var fetchExchangeData = (nodeRuntime) => {
  ...
  if (allAddresses.length > 0) {
    ...
    const req = { url: multiUrl, method: "GET" };
    try {
  ...
  for (const exchange of nodeRuntime.config.exchanges) {
    ...
    const totalReserveEth = walletBalances.reduce((sum, wb) => sum + wb.balanceEth, 0);
    results.push({
      exchangeId: exchange.exchangeId,
      name: exchange.name,
      walletBalances,
...
var readPoRFeed = (runtime2, chainSelector, feedAddress) => {
  ...
  const roundDataResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
  ...
  const descResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: "0x0000000000000000000000000000000000000000",
      to: feedAddress,
      data: descCallData
  ...
  const [, answer, , updatedAt] = decodeFunctionResult({
    abi: AggregatorV3InterfaceABI,
    functionName: "latestRoundData",
    data: roundData.returnData
  });
  const description = decodeFunctionResult({
    abi: AggregatorV3InterfaceABI,
    functionName: "description",
    data: desc.returnData
  });
  const decimals = decodeFunctionResult({
    abi: AggregatorV3InterfaceABI,
    functionName: "decimals",
    data: decimalsData.returnData
  });
  return {
    feedAddress,
    answer,
...
var onCronTrigger = (runtime2) => {
  runtime2.log("VaultSentinel: Solvency check triggered");
  runtime2.log("Fetching exchange wallet balances across oracle nodes...");
  const exchangeData = runtime2.runInNodeMode(fetchExchangeData, consensusMedianAggregation())().result();
  runtime2.log(`Fetched data for ${exchangeData.length} exchanges`);
  const porFeeds = [];
  for (const evmConfig of runtime2.config.evms) {
    ...
    for (const feedAddress of evmConfig.porFeedAddresses) {
      try {
        const feedData = readPoRFeed(runtime2, chainSelector, feedAddress);
        porFeeds.push(feedData);
        runtime2.log(`PoR Feed ${feedData.description}: ${feedData.answer} (decimals: ${feedData.decimals})`);
      } catch (err) {
        runtime2.log(`Failed to read PoR feed ${feedAddress}: ${err}`);
      }
    }
  }
  for (const exchange of exchangeData) {
    runtime2.log(`${exchange.name}: Total Reserve ETH = ${exchange.totalReserveEth.toFixed(4)}, ` + `Wallets tracked = ${exchange.walletBalances.length}`);
  }
  const result = {
  ...
  runtime2.log("VaultSentinel: Solvency check complete");
  return result;
};
